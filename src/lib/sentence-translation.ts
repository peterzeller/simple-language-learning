...
interface NormalizedTimestampWord {
  word: string;
...
function repairTranscriptAgainstSourceText(input: {
  sourceText: string;
  transcriptWords: SentenceWordTimestamp[];
}): SentenceWordTimestamp[] {
  const sourceWords = tokenizeSourceWords(input.sourceText);
  if (sourceWords.length === 0) {
    return input.transcriptWords;
  }

  const transcriptWords = input.transcriptWords
    .map((word) => ({
      ...word,
      normalizedWord: normalizeAlignmentWord(word.word),
    }))
  ...
  const mappedDurations = mapping
    .map((transcriptIndex) => (
      transcriptIndex === null
        ? null
        : Math.max(0.05, transcriptWords[transcriptIndex]!.endSeconds - transcriptWords[transcriptIndex]!.startSeconds)
    ))
  ...
  const repaired = sourceWords.map((word, index) => {
    ...
    if (mappedIndex !== null && transcriptWords[mappedIndex]) {
      ...
      return {
        word,
        startSeconds: mappedWord.startSeconds,
        endSeconds: mappedWord.endSeconds,
      };
    }

    let previousMappedIndex = index - 1;
    while (previousMappedIndex >= 0 && mapping[previousMappedIndex] === null) {
      previousMappedIndex -= 1;
    }

    let nextMappedIndex = index + 1;
    while (nextMappedIndex < mapping.length && mapping[nextMappedIndex] === null) {
      nextMappedIndex += 1;
    }

    const previousWord = previousMappedIndex >= 0
      ? transcriptWords[mapping[previousMappedIndex] as number]
      : undefined;
    const nextWord = nextMappedIndex < mapping.length
      ? transcriptWords[mapping[nextMappedIndex] as number]
      : undefined;
    ...
    if (previousWord && nextWord && nextMappedIndex - previousMappedIndex > 1) {
      ...
      const start = previousWord.endSeconds
        + ((nextWord.startSeconds - previousWord.endSeconds) * (missingPosition - 1)) / Math.max(1, missingCount);
      const end = previousWord.endSeconds
        + ((nextWord.startSeconds - previousWord.endSeconds) * missingPosition) / Math.max(1, missingCount);
      return {
        word,
        startSeconds: Math.max(0, start),
        endSeconds: Math.max(start + 0.05, end),
      };
    }
    ...
    if (previousWord) {
      ...
      return {
        word,
        startSeconds: start,
        endSeconds: start + defaultDuration,
      };
    }
    ...
    if (nextWord) {
      ...
      return {
        word,
        startSeconds: Math.max(0, end - defaultDuration),
        endSeconds: end,
      };
    }
    ...
    return {
      ...
      endSeconds: fallbackStart + defaultDuration,
    };
  });
...
export async function getOrCreateSentenceWordTimestamps(input: {
  ...
  const sentenceRow = await db
    ...
    .where("id", "=", input.sentenceId)
    .executeTakeFirst();
  const sourceText = sentenceRow?.source_text?.trim() ?? "";
  ...
  if (existingTranscript) {
    ...
    if (parsed) {
      ...
      const repairedFromCached = repairTranscriptAgainstSourceText({
        sourceText,
        transcriptWords: parsed,
      });
      if (repairedFromCached.length > 0) {
        return repairedFromCached;
      }
    }
  }
  ...
  const repairedWords = sourceText
    ? repairTranscriptAgainstSourceText({
      sourceText,
  ...
  await db
    ...
    .values({
      sentence_translation_id: input.sentenceId,
      transcript_json: JSON.stringify(repairedWords),
    })
